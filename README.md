...
- **MCP (HTTP)**: `http://127.0.0.1:7777/mcp` con `Authorization: Bearer <apiKey>`.
  33 tools propias — 25 `vm_*`, 5 de ventanas (`windows_list`, `window_show/hide`, `browser_windows_show/hide`),
  3 de servicios (`services_status`, `service_restart`, `services_events`) — más las del Playwright MCP como `browser_*`
  (proxy 1:1; 25 en @playwright/mcp 0.0.82).
