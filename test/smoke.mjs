...
const ses = await api('/sessions'); check('sessions', ses.status === 200 && typeof ses.body.mode === 'string', `modo=${ses.body.mode} bolsa=${ses.body.shared?.cookies} cookies perfiles=${(ses.body.profiles||[]).map(p=>p.id).join(',')||'-'}`);
